...
def main(n=10 ** 4, b=1000, bin_sizes=(10, 1000, 10), colors=None):
    ...
    for i in progress_bar(range(n), prefix="Adding Objects".ljust(fixed_len)):
        ...
        if e1 != e2 and e1:
            print(
                f"[!] ERROR: Did not expect BinNotFoundException for Object - {i} with Size - {size} and Color - {color}")
            return
        elif e1 != e2 and e2:
    ...
    for i in progress_bar(range(n), prefix="Checking Added Objects".ljust(fixed_len)):
        if gcms.object_info(i) != stupid_gcms.object_info(i):
            print(
                f"[!] ERROR: Expected Object Info for Object - {i}: {stupid_gcms.object_info(i)}, Received: {gcms.object_info(i)}")
            return
    ...
    for i in progress_bar(x[:len(x) // 100], prefix="Deleting Objects".ljust(fixed_len)):
        ...
        for j in range(b):
            if gcms.bin_info(j) != stupid_gcms.bin_info(j):
                print(
                    f"[!] ERROR: Expected Bin Info for Bin - {j}: {stupid_gcms.bin_info(j)}, Received: {gcms.bin_info(j)}")
                return
    ...
    for i in progress_bar(range(n), prefix="Adding Objects".ljust(fixed_len)):
        ...
        if e1 != e2 and e1:
            print(
                f"[!] ERROR: Did not expect BinNotFoundException for Object - {i + n} with Size - {size} and Color - {color}")
            return
        elif e1 != e2 and e2:
            print(f"[!] ERROR: Expected BinNotFoundException for Object - {i} with Size - {size} and Color - {color}")
            return
